...
void Xiface::handle_callb(int k, X_window *W, XEvent *E)
{
    ...
    case CB_AUDIO_ACT:
        if (_aupar)
            ...
            _aupar = new M_ifc_aupar(SRC_GUI_DRAG, _audiowin->asect(), _audiowin->parid(), _audiowin->value());
        if (_audiowin->final())
        {
    ...
    case CB_DIVIS_ACT:
        if (_dipar)
            ...
            _dipar = new M_ifc_dipar(SRC_GUI_DRAG, _instrwin->divis(), _instrwin->parid(), _instrwin->value());
        if (_instrwin->final())
        {
