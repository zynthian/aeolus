...
Audio_alsa::Audio_alsa(const char *name, Lfq_u32 *qnote, Lfq_u32 *qcomm, const char *device, int fsamp, int fsize, int nfrag) : Audio(name, qnote, qcomm),
                                                                                                                                _alsa_handle(0)
{
...
void Audio_alsa::thr_main(void)
{
    ...
    while (_running)
    {
        ...
        while (k >= _fsize)
        {
            ...
            for (int i = 0; i < _nplay; i++)
                _alsa_handle->play_chan(i, _outbuf[i], _fsize);
