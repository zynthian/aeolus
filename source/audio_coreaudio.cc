...
Audio_coreaudio::Audio_coreaudio(const char *name, Lfq_u32 *qnote, Lfq_u32 *qcomm, int fsamp, int fsize) : Audio(name, qnote, qcomm),
                                                                                                           _coreaudio_handle(0)
{
    init(fsamp, fsize);
}
...
void Audio_coreaudio::coreaudio_callback(int nframes, AudioBufferList *bufs)
{
    ...
    for (int i = 0; i < _nplay; i++)
        _outbuf[i] = static_cast<float *>(bufs->mBuffers[i].mData);
    proc_synth(nframes);
    proc_mesg();
}
