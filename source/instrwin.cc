...
Instrwin::Instrwin(X_window *parent, X_callback *callb, int xp, int yp, X_resman *xresm) : X_window(parent, xp, yp, XSIZE, YSIZE, Colors.main_bg),
                                                                                           _callb(callb),
                                                                                           _xresm(xresm),
                                                                                           _xp(xp), _yp(yp)
{
...
void Instrwin::setup(M_ifc_init *M)
{
    ...
    if (y < YSIZE)
        y = YSIZE;
    sprintf(s, "Aeolus-%s   Instrument settings", VERSION);
    x_set_title(s);
